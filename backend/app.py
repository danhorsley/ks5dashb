...
def get_school(urn: str):
    ...
    cursor.execute("""
        ...
        ORDER BY e.year
    """, (urn.strip(),))
    data = [{"year": r[0], "entries": r[1], "avg_grade": float(r[2]) if r[2] else 0, "stem": float(r[3]) if r[3] else 0, "arts": float(r[4]) if r[4] else 0, "econ": float(r[5]) if r[5] else 0, "humanities": float(r[6]) if r[6] else 0, "cohort": r[7], "employment": r[8], "school": r[9], "la": r[10], "type": r[11], "min_age": r[12], "max_age": r[13], "total_school_students": r[14], "num_boys": r[15], "num_girls": r[16], "gender": r[17], "is_selective": r[18], "head": r[19], "website": r[20]} for r in cursor.fetchall()]
    conn.close()
    ...
    return {
        "school": urn,
        "data": data,
        "year_13_size": cohort if cohort is not None else "N/A",  # Renamed from "cohort" to "year_13_size"
